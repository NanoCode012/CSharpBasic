...
namespace CSharpBasic_PrimitivetypesExpression
{
	class MainClass
	{
		public static void Main (string[] args)
		{
            ...
            float FindArea(float radius) //for some reason this works :P
			{
				float a;
				a = Pi * (radius * radius);
				return a;
            ...
            byte b = 1; //byte is 1 byte is 8 bit 
            int i = b; //integer is 4 bytes is 32 bit hence Int32
            ...
            catch (Exception)
            {
                Console.WriteLine(  "An error occured. The number didn't "
                                  + "fit the given range"); //to fit screen
            }
